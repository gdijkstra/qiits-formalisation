...
Fam-fam = record
            ...
            ; total = λ { (A , B) (V , W) → (Σ A B) , (λ { (x , y) → Σ (V x) (λ z → W x y z) }) }
            ; proj = λ { (A , B) (V , W) → (λ { (x , y) → x }) , (λ { (x , y) (z , w) → y } ) }
...
-- Fam-dephom = record
--                { DepHom = λ { (X , P) (V , W) → Σ ((x : X) → P x) (λ f → (x : X) → Σ (V x) (λ y → W x (f x) y)) }
--                ; graph = λ { (X , P) (V , W) (f , g) → ((λ x → x , f x) , (λ x y → {!!})) , {!!}}
--                ; snd' = {!!}
--                }
